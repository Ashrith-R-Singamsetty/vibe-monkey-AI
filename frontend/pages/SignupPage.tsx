...
export function SignupPage() {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      await signup({ email, name, password: method === "password" ? password : undefined, method, redirectTo: window.location.origin });
      if (method === "magic") {
        toast({ title: "Magic link sent", description: "Check your inbox to finish sign up." });
        navigate("/magic-sent");
      } else {
      ...
      <Card>
        ...
        <CardContent>
          <form className="space-y-4" onSubmit={onSubmit}>
            ...
            <div className="space-y-2">
              <Label>Method</Label>
              <select className="w-full border rounded-md px-3 py-2 bg-background" value={method} onChange={e => setMethod(e.target.value as any)}>
